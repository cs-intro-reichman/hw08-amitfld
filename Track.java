...
class Track {
    ...
    private String formattedDuration(int totalSeconds) {
        String newDuration, sec;
        int minutes = duration/60, seconds = duration - minutes * 60;
        if (seconds < 10){
            sec = String.format("0%d", seconds);
        } else {
            sec = String.format("%d", seconds);
        }
        newDuration = String.format("%d", minutes) + sec;
        return newDuration;
    }
}